...
const ProductDetails = () => {
  ...
  return !productDetails ? (
    ...
    <>
    <div className="max-w-screen-md mx-auto p-6 bg-white shadow-md rounded-md flex"> 
    ...
    <div className="flex justify-between  bg-white p-1 shadow-md transition-transform transform ">
          {productDetails.images.map((image, index) => (
            <img key={index} src={image} alt={`Product ${index + 1}`} className="w-1/6 h-auto mr-4" />
          ))}
        </div>
    </>
  );
